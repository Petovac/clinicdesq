...
<div style="display:flex;gap:8px;margin-bottom:20px;flex-wrap:wrap;">
    <a href="{{ route('lab.orders.index') }}" class="btn btn-sm {{ !$status ? 'btn-primary' : 'btn-outline' }}">All</a>
    @foreach(['routed' => 'New', 'processing' => 'Processing', 'results_uploaded' => 'Submitted', 'approved' => 'Approved', 'retest_requested' => 'Retest'] as $key => $label)
        <a href="{{ route('lab.orders.index', ['status' => $key]) }}" class="btn btn-sm {{ $status === $key ? 'btn-primary' : 'btn-outline' }}">{{ $label }}</a>
    @endforeach
</div>
        ...
        <table style="width:100%;border-collapse:collapse;font-size:13px;">
            <thead>
                <tr style="background:#f9fafb;border-bottom:1px solid var(--border);">
                    ...
                    <th style="padding:10px 14px;text-align:left;font-size:11px;font-weight:600;color:var(--text-muted);text-transform:uppercase;">Priority</th>
            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid #f3f4f6;">
                    ...
                    <td style="padding:10px 14px;">
                        @foreach($order->tests->take(3) as $t)
                            <span style="background:#eff6ff;color:#1e40af;padding:2px 8px;border-radius:12px;font-size:11px;font-weight:600;">{{ $t->test_name }}</span>
                        @endforeach
                        @if($order->tests->count() > 3)
                            <span style="font-size:11px;color:var(--text-muted);">+{{ $order->tests->count() - 3 }}</span>
                        @endif
                    </td>
                    <td style="padding:10px 14px;">
                        @if($order->priority !== 'routine')
                            <span style="background:#fee2e2;color:#991b1b;padding:2px 8px;border-radius:12px;font-size:10px;font-weight:700;">{{ strtoupper($order->priority) }}</span>
                        @else
