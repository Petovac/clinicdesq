    ...
    <div class="v-card">
        ...
        <form method="POST" action="{{ route('vet.prescription.store', $appointment->id) }}">
            ...
            <div style="display:flex;gap:10px;flex-wrap:wrap;">
                ...
                @if($prescription)
                <button type="button" id="waRxBtn" onclick="sendPrescriptionWhatsApp()" class="v-btn" style="background:#25D366;color:#fff;border:none;">
...
function sendPrescriptionWhatsApp() {
    const btn = document.getElementById('waRxBtn');
    if (!btn) return;
    btn.disabled = true;
    btn.textContent = 'Sending...';
    fetch('/whatsapp/send/prescription/{{ $appointment->id }}', {
        method: 'POST',
        headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Accept': 'application/json', 'Content-Type': 'application/json' },
    })
    .then(r => r.json())
    .then(data => {
        if (data.success) { btn.textContent = '✓ Sent'; btn.style.background = '#16a34a'; }
        else { alert(data.message || 'Failed'); btn.textContent = 'Retry'; btn.disabled = false; }
    })
    .catch(e => { alert('Error: ' + e.message); btn.textContent = 'Retry'; btn.disabled = false; });
}
