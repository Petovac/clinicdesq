...
.btn-secondary {
    background: #e5e7eb; color: #111827; padding: 10px 20px; border: none; border-radius: 8px;
...
<div class="role-form">
    <form method="POST" action="{{ route('organisation.roles.update', $role->id) }}">
        ...
        <div class="form-card">
            ...
            <div class="form-group">
                ...
                <select name="clinic_scope" required>
                    <option value="none" @selected($role->clinic_scope === 'none')>No Clinic (Central Role)</option>
                    <option value="single" @selected($role->clinic_scope === 'single')>Single Clinic (Manager / Receptionist)</option>
                    <option value="multiple" @selected($role->clinic_scope === 'multiple')>Multiple Clinics (Area / Regional)</option>
                </select>
            </div>
        </div>
            ...
            @foreach($groupedPermissions as $group => $perms)
                <div class="perm-card">
                    <div class="perm-card-header">
                        ...
                        <span class="perm-card-toggle" onclick="toggleGroup(this)">
                            {{ $perms->every(fn($p) => in_array($p->id, $rolePermissions)) ? 'Deselect All' : 'Select All' }}
                        </span>
                    </div>
        ...
        <div style="margin-top: 24px; display: flex; gap: 12px;">
            ...
            <a href="{{ route('organisation.roles.index') }}" class="btn-secondary">Cancel</a>
        </div>
    </form>
</div>
