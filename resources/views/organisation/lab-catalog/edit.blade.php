...
<div class="card">
    <form method="POST" action="{{ route('organisation.lab-catalog.update', $test) }}">
        @csrf
        @method('PUT')
        ...
        <div class="form-row">
            <div class="form-group">
                <label>Code</label>
                <input type="text" name="code" value="{{ old('code', $test->code) }}">
            ...
            <div class="form-group">
                ...
                <select name="category">
                    ...
                    @foreach(['hematology','biochemistry','urinalysis','serology','cytology','histopathology','microbiology','immunology','endocrinology','parasitology','other'] as $cat)
                        <option value="{{ $cat }}" {{ old('category', $test->category) == $cat ? 'selected' : '' }}>{{ ucfirst($cat) }}</option>
                    @endforeach
                </select>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <select name="sample_type">
                    ...
                    @foreach(['blood','serum','plasma','urine','swab','tissue','fluid','feces','other'] as $type)
                        <option value="{{ $type }}" {{ old('sample_type', $test->sample_type) == $type ? 'selected' : '' }}>{{ ucfirst($type) }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input type="text" name="estimated_time" value="{{ old('estimated_time', $test->estimated_time) }}" placeholder="e.g. 2 hours">
        ...
        <div class="form-group">
            ...
            <input type="text" name="parameters" value="{{ old('parameters', is_array($test->parameters) ? implode(', ', $test->parameters) : $test->parameters) }}" placeholder="e.g. RBC, WBC, Platelets, Hb">
        ...
        <div class="form-row">
            <div class="form-group">
                <label>Price (&#8377;)</label>
                <input type="number" name="price" value="{{ old('price', $test->price) }}" step="0.01" min="0">
                @error('price') <div class="error-text">{{ $message }}</div> @enderror
            </div>
            <div class="form-group">
                <label>Cost Price (&#8377;)</label>
                <input type="number" name="cost_price" value="{{ old('cost_price', $test->cost_price) }}" step="0.01" min="0">
        ...
        <div class="form-actions">
            <button type="submit" class="btn-primary">Update Test</button>
